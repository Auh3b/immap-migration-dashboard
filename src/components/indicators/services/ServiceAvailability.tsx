...
import { AggregationTypes, _FilterTypes, groupValuesByColumn } from "@carto/react-core";
...
const method:MethodFunc=(input, column, params)=>{
  ...
  const services = new Map(Array.from(new Set(input.map(d => d[column] + '_' + d[column+'_1']))).map((d)=> {
  ...
  for(let valueColumn of valueColumns){
    ...
    for(let [key, service] of Array.from(services)){
      const serviceValue = aggregatedValueByColumn.filter(({name})=> name === key)[0]
      //@ts-ignore
      serviceValue[valueColumn] = serviceValue.value ?? 0
      const outputIndex = aggregatedValueByColumn.findIndex((d)=> +d?.name === key)
      if(outputIndex >= 0){
...
export default function ServiceAvailability({ dataSource }:BasicWidgetType) {
  const { widget } = useWidgetEffect(
    <CustomColumnBarWidget {...props} dataSource={dataSource} />, [dataSource]
  )
  return (
    <Grid item>
      {widget}
