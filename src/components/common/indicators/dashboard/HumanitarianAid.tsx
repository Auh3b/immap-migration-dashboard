...
function pivotData (data:any[], column:string):any[]{
  ...
  const pivotedData = valuesArray.filter((i:string)=> i.length > 0).map((i:any)=> (Object.fromEntries(new Map([[column, i]]))))
...
function useWidgetFilterValues({ dataSource, id, column, type }:{
...
export default function HumanitarianAid() {
  ...
  const {data, isLoading, error } = useWidgetFetch({layerId: "hotspotsLayer", method: pivotData, column, mapRef, source})

  const selectedCategories = useWidgetFilterValues({
    dataSource, column, id, type
  }) || EMPTY_ARRAY
